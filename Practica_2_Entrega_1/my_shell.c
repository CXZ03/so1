...
void DeleteCharArray(char *args, char caracter);
 ...
 *
 * Descripción:
 Funcion que nos permitira cambiar de directorio
 El cd avanzado no esta implementado
 * Argumentos:
 *
 * Salida:
 ...
 *
 * Descripción:
D   Comprueba los agrumentos , si es correcta los abre con fopen, 
...
/**
 * Función: int internal_fg()
 * --------------------------------------
 *
 * Descripción:
  Eliminamos de la lista la funcion, y pasara su estado a estar en 'E' (Foreground)
 ...
 if (args[1]) 
    {
        ...
        if (job_index > 0 && job_index <= NJobs)            
        {
            // Si el trabajo está parado, enviamos la señal para ejectuarlo
            if (jobs_list[job_index].estado == 'D') 
            {
        ...
        (jobs_list[job_index].cmd, '&');
            ...
            jobs_list[0].estado = jobs_list[job_index].estado;
            strcpy(jobs_list[0].cmd, jobs_list[job_index].cmd);
...
int internal_bg(char **args) {
    ...
    printf(
      ...
      if (args[1])  {
        ...
        if (job_index > 0 && job_index < 1){
            if (jobs_list[job_index].estado == 'D')
            {
                strcat(jobs_list[job_index].cmd, " &\0");
                jobs_list[job_index].estado = 'E';
                kill(jobs_list[job_index].pid, SIGCONT);
                ...
                fprintf(stderr,"[%d] %d \t%c \t%s \n", job_index, jobs_list[job_index].pid, jobs_list[job_index].estado, jobs_list[job_index].cmd);
 ...
 * en background) y 0 (FALSE) en caso contrario (comando en 0).
 ...
 * --------------------
 * Descripción: En caso de no comando interno, lo ejecute de manera externa 
    con un fork

 *
 * Argumentos:
 *      - line: orden a ejecutar
 *
 * Salida:
 *   
 */
        ...
        if (check_internal(args)) {
            ...
            if (pid == 0) {
                ...
                if (isBackground) {
                    ...
                    printf("[%d] %d\t%c\t%s\n", NJobs, jobs_list[NJobs].pid, jobs_list[NJobs].estado,
                      jobs_list[NJobs].cmd);
 ...
 * --------------------
 * Descripción:Manejador propio del la señal SIGCHILD cuando uno de los procesos hijos termina
    ...
    while ((end = waitpid(-1, &stat, WNOHANG)) > 0) {
        ...
        if (end == jobs_list[0].pid) {
            ...
            if (WIFEXITED(stat)) {
                ...
                printf("Terminado PID %d (%s) finalizado con exit code %d\n",
                       end,jobs_list[pos].cmd, WEXITSTATUS(stat));

            } else {
                if (WIFSIGNALED(stat)) {

                    printf(
                        "Terminado PID %d (%s) finalizado con exit code %d\n",
                        end,jobs_list[pos].cmd, WTERMSIG(stat));
                }
            }
#if DEBUG_5
            printf(
 ...
 * --------------------
 * Descripción:Manejador propio del la señal SIGNIT
 * Suspende el proceso 
 *
 * Argumentos:
 *      - signum: señal que llama a la función
...
#if DEBUG_5
    printf(
        "\n[ctrlz()→ Soy el proceso con PID %d, el proceso en 0 es %d "
        "(%s)]\n",
 ...
 * --------------------
 * Descripción:
 * Detecta si es un redireccionamiento
 *
 * Argumentos:

 * Salida:
 *      0= si no es un redireccionamiento
        1= si es un redireccionamiento 
 */
int is_output_redirection(char **args)
{
    int out = 0;
    ...
    for (int i = 0; (args[i] != NULL && out == 0); i++)
    {
        if (strcmp(args[i], ">") == 0){
    ...
    if (out == 1)
    {
        int fd = open(args[args1], O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
        dup2(fd, 1);
        close(fd);
    }
    return out;
...
/**
 * Función: void DeleteCharArray(char *args, char caracter)
 * --------------------
 * Descripción:
 * Coge el caracter pasado por parametro y lo elimina del
 */
void DeleteCharArray(char *args, char caracter)
{
    int indx = 0;
    int index2 = 0;
    while (args[indx]){
        if (args[indx] != caracter){
            args[index2] = args[indx];
            index2++;
        }
        indx++;
    }
